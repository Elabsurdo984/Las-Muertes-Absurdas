...
# Definimos un recurso para devolver todas las muertes
class Muertes(Resource):
    def get(self):
        return jsonify(deaths)

class MuerteEspecifica(Resource):
    def get(self, muerte_id):
        if 0 <= muerte_id < len(deaths):
            return jsonify(deaths[muerte_id])
        else:
            return jsonify({'error': "Muerte no encontrada"}), 404
        

api.add_resource(Muertes, '/muertes')
api.add_resource(MuerteEspecifica, '/muertes/<int:muerte_id>')
